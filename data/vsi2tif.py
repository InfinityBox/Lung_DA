...
borrowed from https://github.com/andreped/vsi2tif
"""
...
def vsi_to_bigtiff(path_to_input_file, out_path, bfconvert_bin, compression='LZW', tz=1024, plane=0,
    ...
    if os.path.isfile(path_to_input_file):
        ...
        return None
    path_to_output_file = os.path.join(out_path, input_file_name + ".btf")
    # vsi -> btf
...
def vsi2tif(in_file, results_dict, path_to_bfconvert, java_env_var, args):
    start = time.time()
    results_dict[in_file] = {}
    print('Found img {}'.format(in_file))
    result_vsi2bt = vsi_to_bigtiff(in_file, args.tif_path, path_to_bfconvert, compression=args.compression,
                                   tz=args.tilesize, plane=args.plane, java_env=java_env_var)
    if result_vsi2bt is not None:
        result = bigtiff_to_pyramide_tiff(result_vsi2bt, args.tif_path, compression=args.compression,
                                          tz=args.tilesize,
    ...
    print('Done - the processing took {} seconds'.format(time.time() - start))
    return result
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input", default='/home/ains/An/histologic/test/', help="folder with input files")
    parser.add_argument("--output", default='/home/ains/An/histologic/', help="folder for output files")
